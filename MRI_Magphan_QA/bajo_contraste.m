function [contar] = bajo_contraste( I,centro_x,centro_y )

%I=dicomread('IM-0001-0007');
%h_im=imshow(I,[]);

%A=mat2gray(I,[0 30]);
%figure(2),h_im=imshow(A,[]);
...
line2x=[centro_x+24,centro_x];
line2y=[centro_y+26,centro_y];
line3x=[centro_x+30,centro_x];
line3y=[centro_y-30,centro_y];
line4x=[centro_x-23,centro_x];
line4y=[centro_y-31,centro_y];
...
%%
for m=1:(size(y1)-1)
  if(y1(m+1)<(y1(m)-0.15*y1(m))) | (y1(m+1)>(0.15*y1(m)+y1(m)))
      y1(m+1)=y1(m+1);
      m=m+1;
  else
      y1(m+1)=y1(m);
...
%%
for m=1:(size(y2)-1)
  if(y2(m+1)<(y2(m)-0.3*y2(m))) | (y2(m+1)>(0.3*y2(m)+y2(m)))
      y2(m+1)=y2(m+1);
      m=m+1;
  else
      y2(m+1)=y2(m);
...
%%
for m=1:(size(y3)-1)
  if(y3(m+1)<(y3(m)-0.3*y3(m))) | (y3(m+1)>(0.3*y3(m)+y3(m)))
      y3(m+1)=y3(m+1);
      m=m+1;
  else
      y3(m+1)=y3(m);
...
%%
for m=1:(size(y4)-1)
  if(y4(m+1)<(y4(m)-0.2*y4(m))) | (y4(m+1)>(0.2*y4(m)+y4(m)))
      y4(m+1)=y4(m+1);
      m=m+1;
  else
      y4(m+1)=y4(m);
